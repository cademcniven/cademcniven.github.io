...
const resolveLongVowel = function(prevChar) {
	if ("ぁあかがさざただなはばぱまゃやらゎわゕ".indexOf(prevChar) != -1) return "あ"
...
const toRomaji = function(str) {
	...
	for (const i in str) {
		...
		if (c == 0x30fc) {
			...
			doubleNext = true
		} else if (result.length > 0 && result[result.length - 1] == "n" && "aiueo".indexOf(rom[0]) != -1) {
			result += "'" + rom
		} else if (result.length > 0 && result[result.length - 1] == "i" && rom[0] == "x" && rom[1] == "y") {
			if ((result.length > 1 && result[result.length - 2] == "j") || (result.length > 2 && result[result.length - 2] == "h" && (result[result.length - 3] == "c" || result[result.length - 3] == "s")))
				result = result.substring(0, result.length - 1) + rom.substring(2, rom.length)
			else
				result = result.substring(0, result.length - 1) + rom.substring(1, rom.length)
		} else {
			result += rom
		}
	}
...
const updateResultDisplay = function (resultDisplay, inputText) {
	...
	for (const i in tokens) {
		...
		if (isKnown) {
			...
			var j = 1
			while (meaning_index == -2) meaning_index = meaning_indices[token.word_id / 10 - j++]
		} else {
			clazz = " other"
		...
		if (token.pronunciation != undefined && !isOnlyKana(wordDisplay) && token.pronunciation.length > 0) { // contains non-kana elements
			...
			if (!(reading === wordDisplay)) {
				...
				var endIndex = 1
				while (true) {
					if (reading[reading.length - endIndex] !== displayHiragana[displayHiragana.length - endIndex]) break
					endIndex++
				}
		...
		if (!isPunctuation) {
			...
			if (isKnown || meaning_index < 0)
				...
				builtOutput += '<br/><span class="unknown">Unknown</span>'
			builtOutput += '<span class="note"><a href="http://jisho.org/search/' + token.surface_form + '">Jisho</a>'
			if (isKnown) builtOutput += ' #' + token.word_id
			builtOutput += '</span>'
			builtOutput += '</div>'
			builtOutput += '</span>'
		}　else {
			builtOutput += '<span class="punctuation other">' + wordDisplay + "</span>"
		}
	}
